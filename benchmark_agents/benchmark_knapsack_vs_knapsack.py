"""
Benchmark: Strategic Knapsack vs Strategic Knapsack (both with BD).

Uses parallel_evaluate_multi_agent for fast multi-process evaluation.

Usage:
    cd <repo root>
    uv run python benchmark_agents/benchmark_knapsack_vs_knapsack.py [--episodes 50]
"""
...
from agents.multi_agent_knapsack import MultiAgentKnapsackAgent
...
if __name__ == "__main__":
    ...
    agents = {
        name: MultiAgentKnapsackAgent(env=tmp_env, agent_name=name, enable_bd_bidding=True)
        for name in tmp_env.possible_agents
    }

    print(
        f"Knapsack vs Knapsack (BD={'on' if cfg.multi_agent.bd_enabled else 'off'}, "
    ...
    import json
    out_path = "/tmp/knapsack_vs_knapsack.json"
    with open(out_path, "w") as f:
        json.dump(all_reports, f, indent=2, default=str)
    print(f"\nSaved metrics to {out_path}")

    print("\n" + "=" * 60)
    print("KNAPSACK vs KNAPSACK SUMMARY")
    print("=" * 60)
