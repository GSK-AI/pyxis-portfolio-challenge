...
/**
 * v2 competitive-intelligence board for the replay viewer (classic
 * ReplayAlertsPanel logic): all agents' alerts up to the current step,
 * newest first, with the v2 alert-type gradients.
 */
...
// --- identical message formatting to the classic ReplayAlertsPanel ---
function formatAlertMessage(
  ...
  switch (alert.event_type) {
    ...
    case "clinical_site_deal":
      return `${name} won a clinical site at auction for ${formatCurrency(details.price as number | null | undefined)}`;
    case "be_spend": {
      const count = (details.be_count as number | undefined) ?? 0;
      return `${name} boosted brand equity on ${count} drug${count === 1 ? "" : "s"} in ${alert.therapeutic_area}${indication}`;
    }
    case "dc_spend":
      return `${name} grew market demand in ${alert.therapeutic_area}${indication}`;
    default:
...
export function ReplayIntelligenceBoard({
  alerts,
  currentStep,
  agentDisplayNames,
}: {
  ...
  // Classic: all alerts up to and including current step, newest first.
  ...
  if (relevantAlerts.length === 0) {
    return (
      <EmptyState
        ...
        hint="Agent activity shows up here as the game unfolds"
      />
    );
  }
